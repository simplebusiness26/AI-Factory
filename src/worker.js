...
const PROJECT_SEEDS = [
  { id:'ai-factory', name:'AI Factory', repo:'simplebusiness26/AI-Factory', purpose:'Central operating system for projects, agents, decisions, health and releases.', stage:'MVP', currentState:'Mission Control is live-in-build with Watchtower, Release Factory, Today, Decision Engine and Knowledge Mine being unified.', nextActions:['Verify the live Cloudflare URL','Complete the go-live checklist','Start using Today as the daily control surface'], blockers:[] },
...
async function snapshotAll(db,env,force=false){const projects=await readProjects(db);const enriched=await enrichProjects(projects,db,env,force);const watchtower=buildWatchtower(enriched);return{projects:enriched,summary:makeSummary(enriched,env),watchtower,releases:buildReleaseFactory(enriched),today:buildToday(enriched,watchtower)}}
...
async function handleApi(request,env){
  ...
  if(path==='/api/health'&&request.method==='GET')return json({ok:true,service:'ai-factory',version:'0.3.0-factory',runtime:'cloudflare-workers',database:'d1',githubAuthenticated:Boolean(env.GITHUB_TOKEN),writeProtected:Boolean(env.AI_FACTORY_KEY),systems:['mission-control','watchtower','release-factory','today','decision-engine','knowledge-mine']});
  ...
  const brainMatch=path.match(/^\/api\/projects\/([^/]+)\/brain$/);if(brainMatch&&request.method==='PUT'){const denied=await requireWriteAccess(request,env);if(denied)return denied;const id=decodeURIComponent(brainMatch[1]);const body=await readJson(request);const existing=await env.DB.prepare('SELECT id FROM projects WHERE id=?').bind(id).first();if(!existing)return json({error:'Project not found.'},404);await env.DB.prepare(`UPDATE projects SET current_state=?,next_actions=?,blockers=? WHERE id=?`).bind(cleanString(body.currentState,5000),JSON.stringify(cleanList(body.nextActions)),JSON.stringify(cleanList(body.blockers)),id).run();return json({ok:true})}
  const decisionMatch=path.match(/^\/api\/projects\/([^/]+)\/decisions$/);if(decisionMatch&&request.method==='POST'){const denied=await requireWriteAccess(request,env);if(denied)return denied;const id=decodeURIComponent(decisionMatch[1]);const body=await readJson(request);const decision=cleanString(body.decision,2000);if(!decision)return json({error:'Decision is required.'},400);const existing=await env.DB.prepare('SELECT id FROM projects WHERE id=?').bind(id).first();if(!existing)return json({error:'Project not found.'},404);const record={date:new Date().toISOString().slice(0,10),title:cleanString(body.title,200)||'Decision',decision,reason:cleanString(body.reason,2000)};await env.DB.prepare(`INSERT INTO decisions (project_id,date,title,decision,reason) VALUES (?,?,?,?,?)`).bind(id,record.date,record.title,record.decision,record.reason).run();return json({ok:true,decision:record},201)}
